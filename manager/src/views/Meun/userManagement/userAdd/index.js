...
import userAddStyle from "./index.scss";
    ...
    type === "submit"
      ? this.props.form.validateFields((err, values) => {
          if (!err) {
            ...
            values.upData && values.userId && values.userName && values.userPwd
              ? upDataId(values)
              : message.error("请输入身份");
          }
        })
    ...
    type === "submit"
      ? this.props.form.validateFields((err, values) => {
          if (!err) {
            ...
            values.apiIdentity && values.apiIdentityUr && values.apiIdentityFunc
              ? addApi(values)
              : message.error("请完善输入框的内容");
          }
        })
      : this.props.form.resetFields();
  };
    ...
    type === "submit"
      ? this.props.form.validateFields((err, values) => {
          if (!err) {
            ...
            values.statusId && values.viewId
              ? setStatusView(values)
              : message.error("请完善输入框的内容");
          }
        })
      : this.props.form.resetFields();
