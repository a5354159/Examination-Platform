...
function Look(props) {
  ...
  //   getExam
  // } = props;
  const { examType, questions_type, subjectType, allQuestion ,getAllSubject,examtypes,exo} = props;
  console.log(props)
  let [ind, updataInd] = useState(-1);
  useEffect(() => {
    props.examType();//课程类型
    props.getAllCourse(); //题目类型 
    // getAllExam();
    props.getExam();
...
const MapDispatch = dispatch => ({
  ...
  getExam() {
    dispatch({
      type: "question/getQuestionsTypes"
    });
  }, //1
  // getAllExam() {
  //   dispatch({
